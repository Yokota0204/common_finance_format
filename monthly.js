...
function autoMakeSheet()
{
  if ( thisMonth > 1 ) {
    // 今月分のシートを作成。
    let newSheet = formatSh.copyTo( thisYearBook );
    const shName = debug ? "99" + "月度" : thisMonth + "月度";
    try {
      newSheet.setName( shName ).activate();
      let position = formatSh.getIndex() + 1;
      thisYearBook.moveActiveSheet( position ); // 2番目の位置にシートを移動。
...
function autoCopy()
{
  if( thisMonth > 1 )
  {
    ...
    const genreLength = ( lastRow / 17 ) - 1; // コピーするジャンル数

    var copyRow, copyRange, pasteRange;

    for( let i = 0; i < genreLength; i++ )
    {
      copyRow = thisMonth + 17 * i; // コピー行
...
function remind_deadline()
{
  ...
  const message = u_nl + "おはようございます。"
    + u_nl + "本日は" + shNameLastMon + "の申告締切日です。"
...
function notify_result()
{
  ...
  let message = u_nl + "おはようございます。"
    + u_nl + shNameLastMon + "の申告内容が確定されました。" + u_nl + u_nl;

  // 報酬額の集計
  const sheet = thisYearBook.getSheetByName(shNameLastMon);
